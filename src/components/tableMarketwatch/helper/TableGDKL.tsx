...
const TableGDKL = ()=>{
  const { dataTableThongkeKhopLenh } = useAppSelector((state:RootState )=> state.tableTest);
    return  ( 
        <> 
         <table
        id="tbHIST_INDEX"
      ...
      >
        ...
        <tbody>
          {dataTableThongkeKhopLenh ? dataTableThongkeKhopLenh.map((item:any,index:number)=>{
                return <tr key={index}> 
